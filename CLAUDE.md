...
- `src/lib/ads.ts` — `ADSENSE_CLIENT_ID` (currently a placeholder `ca-pub-0000000000000000`) + `loadAdsenseScript`/`isAdsConfigured`. Swap the placeholder for a real publisher ID to go live — everything downstream (AdSlot, ads.txt) is already wired to it.
- `src/components/AdSlot.tsx` — renders a real AdSense `<ins class="adsbygoogle">` unit once `isAdsConfigured()` is true AND the visitor has accepted the cookie-consent banner (`readStorage('mergedoc:ad-consent')`); otherwise falls back to the original dashed placeholder box so layout/spacing stays visible pre-launch.
- `src/components/ConsentBanner.tsx` — bottom cookie-consent banner (Accept/Decline), shown once until a choice is stored in `localStorage['mergedoc:ad-consent']`. Accepting calls `loadAdsenseScript()`; declining just records the choice and shows no ads. Rendered globally from `Layout.tsx`.
- `public/ads.txt` — placeholder AdSense verification file (`pub-0000000000000000`) — must be updated with the real publisher ID alongside `ads.ts`.
- `src/pages/PrivacyPolicy.tsx` — static privacy policy page (client-side processing, localStorage usage, AdSense disclosure + opt-out link). Linked from the footer in `Layout.tsx`, not listed in `tools.ts`/nav (it's not a tool).
